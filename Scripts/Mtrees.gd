...
func _on_grass_is_ready():
	...
	#f_noise.seed = 2
	#var t_noise = terrain.terrain_material.get("shader_parameters/tree1").noise
	#var t_noise = 
	...
	for x in range(get_width()):
		for y in range(get_height()):
			...
			#has_grass = has_grass and n > 0.33 and n < 0.66
			has_grass = has_grass and terrain.ran_arr[Vector2i(x,y)] == 0
