...
func _physics_process(delta):
	...
	match state_machine.get_current_node():
		"walk":
			...
			#if (target_point - global_position).length() <= 1.0:
				#target_point =  Vector3(x,global_position.y,z) + (target_point - global_position).normalized() *2.0
		...
		"stand":
			anim_tree.advance(delta * 1.5) #controls speed of animation
		"run":
